function [featvec, filtvec] = MR8(im)
...
ims = cell(1, 8);
[R,C] = size(im);
imSize = size(im)-[50 50];
filtvec = zeros(38,prod(imSize));
filtI = 1;
i=1;
n=2;
...
for j=0:2,
    for k=0:5,
        ...
        filtvec(filtI,:) = reshape(im1(26:R-25,26:C-25),1,size(filtvec,2)); filtI=filtI+1;
        filtvec(filtI,:) = reshape(im2(26:R-25,26:C-25),1,size(filtvec,2)); filtI=filtI+1;
...
ims{i} = n1 .* (im1+im2);
filtvec(filtI,:) = reshape(ims{i}(26:R-25,26:C-25),1,size(filtvec,2)); filtI=filtI+1;
i=i+1;
ims{i} = n2 .* anigauss(in, sigma, sigma);
filtvec(filtI,:) = reshape(ims{i}(26:R-25,26:C-25),1,size(filtvec,2));
